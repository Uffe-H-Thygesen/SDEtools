...
We interpret this $X_t$ as the position of an animal moving in one dimension. We see that so far - i.e., without killing - the position is an Ornstein-Uhlenbeck process with stationary mean 0, variance $1/2$, and decorrelation time 1. We can verify the stationary distribution numerically as follows, introducing reflective boundaries at the boundaries of a computational domain. Here $D(x) = g^2(x)/2 = 1/2$ is the diffusivity, and  $v(x)=f(x)=-x$ is the advective velocity which equals the drift, since the diffusivity is constant.  
